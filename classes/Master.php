...
Class Master extends DBConnection {
	...
	function save_supplier(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Tên nhà cung cấp đã tồn tại.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id)){
				$res['msg'] = "Nhà cung cấp mới đã được lưu thành công.";
				$id = $this->conn->insert_id;
			}else{
				$res['msg'] = "Nhà cung cấp được cập nhật thành công.";
			}
	...
	function delete_supplier(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Nhà cung cấp được xoá thành công.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_item(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Mặt hàng tồn tại theo nhà cung cấp đã chọn.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Mục mới đã được lưu thành công");
			else
				$this->settings->set_flashdata('success',"Mục mới đã được cập nhật.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_item(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Mục mới được xoá thành công.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_po(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Đơn đặt hàng mới đã được tạo thành công.");
			}else{
				$this->settings->set_flashdata('success'," Đơn đặt hàng mới đã được cập nhật thành công.");
			}
		}
	...
	function delete_po(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Đã xoá thành công");
			if($bo->num_rows > 0){
	...
	function save_receiving(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Hàng mới đã được nhận thành công.");
			}else{
				$this->settings->set_flashdata('success'," Đã cập nhật thành công.");
			}
		}
	...
	function delete_bo(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Xoá thành công");
	...
	function save_return(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Tạo thành công.");
			}else{
