...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title">Chi tiết đơn hàng đã nhận - <?php echo $po_code ?></h4>
    </div>
    <div class="card-body" id="print_out">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6">
                    <label class="control-label text-info">Từ mã đơn hàng</label>
                    <div><?php echo isset($po_code) ? $po_code : '' ?></div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="supplier_id" class="control-label text-info">Nhà cung cấp</label>
                        <div><?php echo isset($supplier) ? $supplier : '' ?></div>
                    </div>
                </div>
                <?php if(isset($bo_id)): ?>
                <div class="col-md-6">
                    <label class="control-label text-info">Từ đơn hàng dự trữ</label>
                    <div><?php echo isset($bo_code) ? $bo_code : '' ?></div>
                </div>
                <?php endif; ?>
            </div>
            <h4 class="text-info">Đơn đặt hàng</h4>
            <table class="table table-striped table-bordered" id="list">
                ...
                <thead>
                    <tr class="text-light bg-navy">
                        ...
                        <th class="text-center py-1 px-2">Mục</th>
                ...
                <tfoot>
                    <tr>
                        <th class="text-right py-1 px-2" colspan="4">Tổng tiền</th>
                        <th class="text-right py-1 px-2 sub-total"><?php echo number_format($total,2)  ?></th>
                    </tr>
                    <tr>
                        <th class="text-right py-1 px-2" colspan="4">Giảm giá
                            <?php echo isset($discount_perc) ? $discount_perc : 0 ?>%</th>
                        <th class="text-right py-1 px-2 discount">
                            <?php echo isset($discount) ? number_format($discount,2) : 0 ?></th>
                    </tr>
                    <tr>
                        <th class="text-right py-1 px-2" colspan="4">Thuế
                            <?php echo isset($tax_perc) ? $tax_perc : 0 ?>%
                        </th>
                        <th class="text-right py-1 px-2 tax"><?php echo isset($tax) ? number_format($tax,2) : 0 ?></th>
                    </tr>
                    <tr>
                        <th class="text-right py-1 px-2" colspan="4">Tổng cộng</th>
                        <th class="text-right py-1 px-2 grand-total">
                            <?php echo isset($amount) ? number_format($amount,2) : 0 ?></th>
                    </tr>
                </tfoot>
            </table>
            <div class="row">
                ...
                <?php if($status > 0): ?>
                <div class="col-md-6">
                    <span class="text-info"><?php echo ($status == 2)? "ĐÃ NHẬN" : "ĐANG GIAO" ?></span>
                </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="card-footer py-1 text-center">
        <button class="btn btn-flat btn-success" type="button" id="print">In</button>
        <a class="btn btn-flat btn-primary"
            href="<?php echo base_url.'/admin?page=receiving/manage_receiving&id='.(isset($id) ? $id : '') ?>">Sửa</a>
        <a class="btn btn-flat btn-dark" href="<?php echo base_url.'/admin?page=receiving' ?>">Quay lại</a>
    </div>
</div>
